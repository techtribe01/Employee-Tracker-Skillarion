...
import { AlertCircle, Eye, EyeOff, Lock, Users, Clock, MapPin, Bell, Calendar } from 'lucide-react'
import { createEvent } from '@/lib/calendar-event-actions'
...
export function CreateEventModal({
  ...
  const [formData, setFormData] = useState({
    ...
    reminderMinutes: 15,
    attendeeEmails: '',
    daysToSchedule: 1,  // NEW: 1, 2, or 3 days
  })
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    console.log('[v0] Creating event with days:', formData.daysToSchedule)
    ...
    const result = await createEvent({
      ...
      reminderMinutes: formData.reminderMinutes,
      attendeeEmails: attendeeEmails.length > 0 ? attendeeEmails : undefined,
      daysToSchedule: formData.daysToSchedule,  // NEW
    })
    ...
    console.log('[v0] Event created, triggering calendar refresh')
    ...
    // Trigger calendar refresh via SWR
    mutate(key => typeof key === 'string' && key.includes('calendar-events'), undefined)
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          {/* NEW: Days to Schedule */}
          <div className="space-y-2">
            <Label htmlFor="daysToSchedule" className="flex items-center gap-2">
              <Calendar className="h-4 w-4" />
              Schedule for multiple days
            </Label>
            <Select
              value={formData.daysToSchedule.toString()}
              onValueChange={value => setFormData({ ...formData, daysToSchedule: parseInt(value) })}
            >
              <SelectTrigger>
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
                <SelectItem value="1">1 day only</SelectItem>
                <SelectItem value="2">2 consecutive days</SelectItem>
                <SelectItem value="3">3 consecutive days</SelectItem>
              </SelectContent>
            </Select>
            <p className="text-xs text-muted-foreground">
              Event will be created for the specified start date plus the selected days
            </p>
          </div>
